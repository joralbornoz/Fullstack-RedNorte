...
export default function LoginPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      // ✅ CORREGIDO: Adaptamos el payload de salida para que use 'password' en vez de 'contrasena'.
      // De esta forma tu DTO en Spring Boot recibirá la clave correctamente mapeada.
      const payloadCredenciales = {
        rut: form.rut,
        contrasena: form.contrasena // 👈 Antes decía 'password', cámbialo a 'contrasena'
      };

      const response = await authService.login(payloadCredenciales)
      // Tolerancia arquitectónica: Extraemos el cuerpo real sin importar si viene crudo o envuelto en un wrapper 'data'
      ...
      // Guardamos oficialmente el token limpio e independiente en el almacenamiento del navegador
      if (userData.token || userData.accessToken) {
        localStorage.setItem('rednorte_token', userData.token || userData.accessToken);
      }

      // Seteamos el estado global en tu AuthContext
      login(userData)

      // Ruteo dinámico estricto por roles
      const destinos = {
        ADMIN: '/admin',
        MEDICO: '/medico',
        PACIENTE: '/paciente'
  ...
  // Shortcuts de desarrollo alineados con el LocalStorage unificado
  function devLogin(rol) {
    ...
    localStorage.setItem('rednorte_token', mock[rol].token);
    login(mock[rol])
